...
```bash
$ git clone https://github.com/quickfever/runcloud-letsencrypt.git && cd runcloud-letsencrypt && sudo mv sple.sh /usr/local/bin/rwssl && sudo chmod +x /usr/local/bin/rwssl && (crontab -l ; echo "@monthly \"sudo service nginx-sp stop && yes | letsencrypt --standalone renew &>/dev/null && service nginx-rc start && service nginx-rc reload\"")| crontab - && service cron reload
```
